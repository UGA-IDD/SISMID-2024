...
demography <- generate_pop_demography(N, times, age_min=0, prob_removal=0, aux=aux)
...
tmp <- res$observed_biomarker_states %>% filter(b == 1)
tmp2 <- res$observed_biomarker_states %>% filter(b == 2)
plot_obs_biomarkers_one_sample(tmp)  
plot_obs_biomarkers_one_sample(tmp2)
```


This is the final dataset I am using for SISMID
```{r}
t = time_data =120

true240 <-res$observed_biomarker_states %>% filter(t==time_data) %>% select(-t)
dem240 <-res$demography %>% filter(times==time_data) %>% 
  mutate(age=times-birth) %>% select(i, age, group)
df1 <- left_join(true240, dem240, by=c("i")) %>% 
  mutate(b=ifelse(b==1, "DP_antibody", "PT_antibody")) %>%
  select(-value) %>%
  pivot_wider(names_from=b, values_from=observed)

df2 <- res$immune_histories_long %>% filter(t<=time_data) %>% 
  filter(value==1) %>% #1 means successful exposure
  group_by(i, x) %>% 
  summarise(no_events = n()) %>%
  mutate(event = ifelse(x==1, "DP_ifxn", ifelse(x==2, "PT_ifxn", "DP_vacc"))) %>%
  mutate(event_yes = ifelse(no_events>=1, 1, 0)) %>%
  select(i, event, event_yes) %>%
  pivot_wider(names_from=event, values_from=event_yes) 
  
df <- left_join(df1, df2, by=c("i")) %>% 
  mutate(DP_ifxn = ifelse(is.na(DP_ifxn), 0, DP_ifxn),
         PT_ifxn = ifelse(is.na(PT_ifxn), 0, PT_ifxn),
         DP_vacc = ifelse(is.na(DP_vacc), 0, DP_vacc)) %>%
  mutate(group = ifelse(group==1, "urban", "rural")) %>%
  select(-i) %>%
  rename(age_months = age) %>%
  select(-c("PT_antibody", "PT_ifxn")) %>%
  rename(DP_infection = DP_ifxn)

write_csv(df, file="/Users/winter/Dropbox/SISMID/data/Diptheria_IgG_Serology.csv")

```


Diphtheria IgG Antibody Concentration:
< 0.01 IU/mL  - highly susceptible to disease
0.01 and 0.09 IU/mL basic levels of protection against disease
>=0.1 IU/mL long term protection
