...
2. Open the data files in a text editor application and determine he delminiter of each file.
...
If your R Project is not already open, go ahead and open it so we take advantage of it setting a useful working directory for us in order to import data.
...
df <- read.csv(file = "~/Dropbox/SISMID/data/serodata.csv") #absolute path starting from my home directory
```


Note #1, I assigned the data frame to an object called `df`.  I could have called the data frame anything, but in order to use the data (i.e., as an object we can find in the Global Environment), I need to assign it to an object. 

Note #2, Look to the Global Environment panel, you will see the df object ready to be used.
...
To install a package you can i) go to Tools ---\> Install Packages in the RStudio header, or ii) use the following code:
```{r echo=TRUE, eval=FALSE}
...
```{r echo=TRUE, eval=FALSE}
df <- read_xlsx(path = "~/Dropbox/SISMID/data/serodata.xlsx", sheet = "Data")
```


## Lets make some mistakes

1. What if we read in the data without assinging it to an object (i.e., `read_xlsx(path = "data/serodata.xlsx", sheet = "Data")`)?

2. What if we forget to specify the sheet argument? (i.e., `dd <- read_xlsx(path = "data/serodata.xlsx")`)?
...
Note, I wrote the data to new file names.  Even though we didnt change the data at all in this module, it is good practice to keep raw data raw, and not to write over it.
